...
if '%setup%'=='True' (
	...
	rsync -r -e "ssh -i %wpkgidfile% -o UserKnownHostsFile=%wpkg_hosts%" --partial --times --timeout=120 %bwlimit% %exclude% --log-file=%wpkglogfile% %wpkgremote%:/opt/updates/ %wpkglocal%/ > nul 2>&1
...
rem /T change Tree (recurse) /Grant, send error out and cmd out to the log file
icacls %ProgramData%\wpkg /T /grant "Administrators":F "SYSTEM":F >> "%temp%\%scriptname%_acl.log" 2>&1
set icacls=%errorlevel%

rem if there was an error, then append the error log to the end of the logfile (might not work in setup mode)
if not %takeown%.%icacls%==0.0 (
